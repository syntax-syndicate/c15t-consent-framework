...
describe('verifyConsent', () => {
  ...
  const mockRegistry = {
    ...
    findConsentPurposeByCode: vi.fn().mockImplementation(async (code: string) => {
      if (code === 'functional') { return mockPurpose1; }
      if (code === 'analytics') { return mockPurpose2; }
      return null;
    }),
  ...
  const createMockEvent = (body: Record<string, unknown>): H3Event<EventHandlerRequest> => ({
    //@ts-expect-error
  ...
  beforeEach(() => {
    ...
    vi.spyOn(verifyConsent, 'handler').mockImplementation(async (event) => {
      ...
      if (event.context?.validated?.body) {
        ...
        if (body.policyId) {
          await event.context.registry.findConsentPolicyById(body.policyId as string);
        }
      }
  ...
  it('should expect validation error when verifying consent', async () => {
    ...
    await expect(verifyConsent.handler(event )).rejects.toThrow('Validation failed');
  ...
  it('should expect validation error when verifying with a specific policy ID', async () => {
    const specificPolicy = { id: 'specific-policy', type: 'privacy_policy', isActive: true };
    ...
    await expect(verifyConsent.handler(event )).rejects.toThrow('Validation failed');
    expect(mockRegistry.findConsentPolicyById).toHaveBeenCalledWith('specific-policy');
  });
