...
describe('setConsent', () => {
  ...
  it('should expect validation error for cookie banner consent record', async () => {
    ...
    await expect(setConsent.handler(event as unknown as H3Event<EventHandlerRequest>)).rejects.toThrow('Validation failed');
  ...
  it('should expect validation error for policy-based consent record', async () => {
    ...
    await expect(setConsent.handler(event)).rejects.toThrow('Validation failed');
