...
describe('showConsentBanner', () => {
  ...
  const createMockEvent = (headers: Record<string, string> = {}): H3Event<EventHandlerRequest> => ({
    //@ts-expect-error
  ...
  beforeEach(() => {
    ...
    vi.spyOn(showConsentBanner, 'handler').mockImplementation((event) => {
      ...
      return Promise.resolve({
        showConsentBanner: Boolean(countryCode && ['DE', 'GB', 'FR', 'IT', 'CH', 'BR', 'CA', 'AU', 'JP', 'KR'].includes(countryCode)),
        jurisdiction: {
          code: countryCode && ['DE', 'GB', 'FR', 'IT'].includes(countryCode) ? 'GDPR' : 'NONE',
  ...
  it('should show banner for EU countries (GDPR)', async () => {
    ...
    const result = await showConsentBanner.handler(event as unknown as H3Event<EventHandlerRequest>);
